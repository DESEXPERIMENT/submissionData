...
def mainProc():
	...
	while 1:
		...
		if ietc%10 == 0 or ietc == 0:
			...
			authBody = 'grant_type=refresh_token&refresh_token=d74WAbGqU3M4enr23hRCRywlt2XuS%2B%2B0ngkQjU8cEgTTK2NiAkGyfmz1XqfsI2uOiOx1TflQtFNVhw8ld5vY7ejnUBX9brPVq3nqvazMMOpw'
		...
		conn.request('GET', '/v1/last-mile/nearby-rideables?origin_lat=38.922875767229904&origin_lng=-76.99619337916373&radius_km=220.260264520463144&result_types=rideables&result_types=stations','',headersDynamic)
		...
		if response.status == 200:
			content = gzip.GzipFile(fileobj=BytesIO(content_raw)).read().decode('utf-8')
			print(ietc,'(started at '+nowf3.strftime('%I:%M %p')+'): Lyft- DC - Response Length (gzip compressed): ',len(content_raw))
			#write the response 
