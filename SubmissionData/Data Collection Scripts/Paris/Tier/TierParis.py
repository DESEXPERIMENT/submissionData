...
if __name__ == "__main__":
	...
	while  1:
		...
		conn.request('GET', '/vehicle?lat=48.864884&lng=2.340593&radius=400000','',headersDynamic)
		...
		if 'Content-Encoding' in response_headers.keys():
			print(response.status, (response_headers))
			content = gzip.GzipFile(fileobj=BytesIO(content_raw)).read().decode('utf-8')
			print(len(content))
			nowf2 = datetime.datetime.now()
			filenm = dpath +"Paris/Data-responseEncoded-"+nowf2.strftime("%Y-%m-%d")+".txt"
			...
			print(ietc,'Starters at:',nowf3.strftime('%I:%M %p'),': Tier Paris')
			ietc +=1
			if(ietc > 100):
				ietc = 0
				print('longer sleep')
