...
myCitiesList = [           'Brussels',         'Berlin',             'Chicago',                 'DC',               'Detroit',            'Lisbon',             'Paris',              'SF']
	...
	while 1:
		...
		with Manager() as manager:
			...
			for i in range(numberOfThreads):
				p = Process(target=worker, args=(7,dataArrayLock,d,reqContent,myCityCoordinates[2*7],myCityCoordinates[(2*7) + 1], myCitiesList[7], ietNumber))
				procs.append(p)
			current_milli_timeLife2 = int(round(time.time() * 1000))
			for i in range(numberOfThreads):
				procs[i].start()
