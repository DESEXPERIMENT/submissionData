...
headersDynamic = {}
cityCoords = []
leftCol = [42.442151, -83.219174 ,42.277179, -83.208507]
rightCol = [42.360036, -82.931613 , 42.449662, -82.912779]
...
def worker(tid, reqContent,d):
	...
	if response.status == 200:
		...
		#	print(tid, len(content))
			...
			logFile.write(b'0x0A')
			logFile.close()
			#print(tid, 'Exception', content_raw)
			d[tid] = 222
	else:
...
def getToken():
	...
	conn.request('POST', '/api/v1/auth_tokens','{"email":{"email":"hh.hassan@gmail.com","password":"baboon123"},"isApplePayDefault":false,"grantType":"email"}',headersDynamic)
...
#		print(2*i,(2*i)+1)
	...
	while 1:
		with Manager() as manager:
			...
			print(ietc,'(started at '+nowf3.strftime('%I:%M %p')+'): Spin - Detroit')
			ietc += 1
			if(ietc % 20 == 0):
				getToken()
			outofrgn = 0
			for x in range(len(d)):
				if(d[x] == 222):
					outofrgn += 1
			print('No scootie available in ', outofrgn, 'out of ',int(len(cityCoords)/2))
			time.sleep(41)
