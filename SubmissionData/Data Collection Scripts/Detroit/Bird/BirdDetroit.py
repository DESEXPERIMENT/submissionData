...
myCitiesList = [           'Detroit',         'Detroit',         'Detroit', 'Detroit', 'Detroit']
...
myCityUrl = [
#DC
'/bird/nearby?latitude=42.33155059814453&longitude=-83.04663848876953&radius=100000.0', 
'/bird/nearby?latitude=42.34255059814453&longitude=-83.03063848876953&radius=100000.0', 
'/bird/nearby?latitude=42.35255059814453&longitude=-83.06663848876953&radius=100000.0', 
'/bird/nearby?latitude=42.32155059814453&longitude=-83.06663848876953&radius=100000.0', 
'/bird/nearby?latitude=42.31155059814453&longitude=-83.00663848876953&radius=100000.0', 
]
myCityLoc = [
#DC
'{"accuracy":1.4726860523223877,"altitude":57.0,"heading":null,"latitude":42.33155059814453,"longitude":-83.04663848876953,"mocked":false,"speed":0.0}', 
'{"accuracy":1.4726860523223877,"altitude":57.0,"heading":null,"latitude":42.34255059814453,"longitude":-83.03063848876953,"mocked":false,"speed":0.0}', 
'{"accuracy":1.4726860523223877,"altitude":57.0,"heading":null,"latitude":42.35255059814453,"longitude":-83.06663848876953,"mocked":false,"speed":0.0}', 
'{"accuracy":1.4726860523223877,"altitude":57.0,"heading":null,"latitude":42.32155059814453,"longitude":-83.06663848876953,"mocked":false,"speed":0.0}', 
'{"accuracy":1.4726860523223877,"altitude":57.0,"heading":null,"latitude":42.31155059814453,"longitude":-83.00663848876953,"mocked":false,"speed":0.0}', 
...
def worker(tid, dataArrayLock, d, reqContent, City, ietNumber):
	...
	if response.status == 200:
		try:
			...
			print(tid, ietNumber,'-', City,'- Length:', len(content))
...
def readResponseUber(tid, response):
	...
	filenm = dpath +'Detroit'+"/P-"+str(tid)+"-responseEncoded-"+nowf2.strftime("%Y-%m-%d")+".txt"
...
def mainProc():
	...
	while 1:
		...
		with Manager() as manager:
			...
			#print(reqContent)	
			print(ietNumber," ~~[Detroit]~~ - Started at:",nowf.strftime('%I:%M %p'),"-In Time M:", int(((int(round(time.time() * 1000)) - current_milli_timeLife)/1000)/60),"-S:", int(((int(round(time.time() * 1000)) - current_milli_timeLife)/1000)%60))	
			ietNumber += 1
			mainCount += 1
