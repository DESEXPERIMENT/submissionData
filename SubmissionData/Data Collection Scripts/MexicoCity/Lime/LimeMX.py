...
myCitiesList = [           'Brussels',         'Berlin',             'Chicago',                 'DC',               'Detroit',            'Lisbon',             'Paris',              'SanFrancisco' , 'Tel Aviv', 'MX']
...
leftCol = [19.457284, -99.189005 , 19.405449, -99.185512]
rightCol = [19.453991, -99.130729, 19.404721, -99.125916]
...
authCodes = ['Bearer eyJ0eXAiOiJKV1QiLCJhbGciOiJIUzI1NiJ9.eyJ1c2VyX3Rva2VuIjoiUElNUlBPSktRNTZVNiIsImxvZ2luX2NvdW50IjoxfQ.fDb3xDNoW69A6vQ_rDCzB39jtudd2Qz4QAACQI5WnF0']
def fillHeaders(tid):
...
def worker(tid, dataArrayLock, d, reqContent, authCode):
	...
	#print(tid, citySquares[tid].upperRight_ne_x	, citySquares[tid].upperRight_ne_y)
	...
	if response.status == 200:
		...
		content = str(citySquares[tid].upperRight_ne_x)+','+str(citySquares[tid].upperRight_ne_y)+'--'+str(milis)+'--'+content
...
def readResponseUber(tid, response):
	...
	filenm = dpath +'MX'+"/P-"+str(tid)+"-responseEncoded-"+nowf2.strftime("%Y-%m-%d")+".txt"
...
def mainProc():
	...
	while ik < 24:
		...
		with Manager() as manager:
			...
			#print(reqContent)	
			print(ietNumber,"- ~~[Mexico City]~~ Started at:",nowf.strftime('%I:%M %p'),"-In Time M:", int(((int(round(time.time() * 1000)) - current_milli_timeLife)/1000)/60),"-S:", int(((int(round(time.time() * 1000)) - current_milli_timeLife)/1000)%60))	
			ietNumber += 1
			mainCount += 1
			time.sleep(0.89)
			ik += 3
