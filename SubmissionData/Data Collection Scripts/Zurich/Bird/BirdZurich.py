...
myCitiesList = [           'Zurich',         'Zurich',         'Zurich', 'Zurich', 'Zurich']
...
myCityUrl = [
#DC
'/bird/nearby?latitude=47.37239456176758&longitude=8.54233169555664&radius=100000.0',
'/bird/nearby?latitude=47.376158&longitude=8.528082&radius=100000.0',
'/bird/nearby?latitude=47.375724&longitude=8.548315&radius=100000.0',
'/bird/nearby?latitude=47.370037&longitude=8.555952&radius=100000.0',
'/bird/nearby?latitude=47.363207&longitude=8.531932&radius=100000.0'
]
myCityLoc = [
#DC
'{"accuracy":1.600294828414917,"altitude":57.0,"heading":null,"latitude":47.37239456176758,"longitude":8.54233169555664,"mocked":true,"speed":0.0}', 
'{"accuracy":1.3924957513809204,"altitude":57.0,"heading":null,"latitude":47.376158,"longitude":8.528082,"mocked":true,"speed":0.0}',
'{"accuracy":1.6441841125488281,"altitude":57.0,"heading":null,"latitude":47.375724,"longitude":8.548315,"mocked":true,"speed":0.0}',
'{"accuracy":1.5051865577697754,"altitude":57.0,"heading":null,"latitude":47.370037,"longitude":8.555952,"mocked":true,"speed":0.0}',
'{"accuracy":1.5797275304794312,"altitude":57.0,"heading":null,"latitude":47.363207,"longitude":8.531932,"mocked":true,"speed":0.0}',
]
serverNumber = 0;
tid1= None
procs=[]
...
def worker(tid, dataArrayLock, d, reqContent, City, ietNumber):
	...
	if response.status == 200:
		...
		if response.status !=307 and response.status!=429:
			print(tid,'-', City,'- Length:', len(content_raw))
...
def readResponseUber(tid, response):
	...
	filenm = dpath +'Zurich'+"/P-"+str(tid)+"-responseEncoded-"+nowf2.strftime("%Y-%m-%d")+".txt"
...
def mainProc():
	...
	while 1:
		...
		with Manager() as manager:
			...
			#print(reqContent)	
			print(ietNumber," ~~[Zurich]~~ - Started at:",nowf.strftime('%I:%M %p'),"-In Time M:", int(((int(round(time.time() * 1000)) - current_milli_timeLife)/1000)/60),"-S:", int(((int(round(time.time() * 1000)) - current_milli_timeLife)/1000)%60))	
			ietNumber += 1
			mainCount += 1
