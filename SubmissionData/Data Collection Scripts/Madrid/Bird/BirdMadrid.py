...
myCitiesList = [           'Madrid',         'Madrid',         'Madrid', 'Madrid', 'Madrid', 'Madrid']
...
myCityUrl = [
#DC
'/bird/nearby?latitude=40.41670608520508&longitude=-3.7035820484161377&radius=100000.0', 
'/bird/nearby?latitude=40.429020&longitude=-3.721159&radius=100000.0',
'/bird/nearby?latitude=40.445061&longitude=-3.685710&radius=100000.0',
'/bird/nearby?latitude=40.456904&longitude=-3.701623&radius=100000.0',
'/bird/nearby?latitude=40.400377&longitude=-3.687806&radius=100000.0',
'/bird/nearby?latitude=40.445624&longitude=-3.732239&radius=100000.0'
]
myCityLoc = [
#DC
'{"accuracy":1.5165560245513916,"altitude":57.0,"heading":null,"latitude":40.41670608520508,"longitude":-3.7035820484161377,"mocked":false,"speed":0.0}', 
'{"accuracy":1.3924957513809204,"altitude":57.0,"heading":null,"latitude":40.429020,"longitude":-3.721159,"mocked":true,"speed":0.0}',
'{"accuracy":1.6441841125488281,"altitude":57.0,"heading":null,"latitude":40.445061,"longitude":-3.685710,"mocked":true,"speed":0.0}',
'{"accuracy":1.5051865577697754,"altitude":57.0,"heading":null,"latitude":40.456904,"longitude":-3.701623,"mocked":true,"speed":0.0}',
'{"accuracy":1.5797275304794312,"altitude":57.0,"heading":null,"latitude":40.400377,"longitude":-3.687806,"mocked":true,"speed":0.0}',
'{"accuracy":1.5797275304794312,"altitude":57.0,"heading":null,"latitude":40.445624,"longitude":-3.732239,"mocked":true,"speed":0.0}'
]
serverNumber = 0;
tid1= None
procs=[]
...
def worker(tid, dataArrayLock, d, reqContent, City, ietNumber):
	...
	if response.status == 200:
		try:
			...
			print(tid,'-', City,'- Length:', len(content))
			content = str(tid)+'--'+str(milis)+'--'+content
...
def mainProc():
	...
	while 1:
		...
		with Manager() as manager:
			...
			#print(reqContent)	
			print(ietNumber," ~~[Madrid]~~ - Started at:",nowf.strftime('%I:%M %p'),"-In Time M:", int(((int(round(time.time() * 1000)) - current_milli_timeLife)/1000)/60),"-S:", int(((int(round(time.time() * 1000)) - current_milli_timeLife)/1000)%60))	
			ietNumber += 1
			mainCount += 1
