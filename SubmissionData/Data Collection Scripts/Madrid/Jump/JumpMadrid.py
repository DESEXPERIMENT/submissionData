...
myCitiesList = [           'Brussels',         'Madrid',             'Chicago',                 'DC',               'Detroit',            'Lisbon',             'Paris',              'SanFrancisco']
myCityCoordinates = [50.847140, 4.352107, 40.416618, -3.703787, 41.845225, -87.690688, 38.913201, -77.011789, 42.374334, -83.077858, 38.722195, -9.139227, 48.864884, 2.340593 , 37.753861, -122.442754 ]
	...
	while 1:
		...
		with Manager() as manager:
			...
			for i in range(numberOfThreads):
				p = Process(target=worker, args=(1,dataArrayLock,d,reqContent,myCityCoordinates[2*1],myCityCoordinates[(2*1) + 1], myCitiesList[1], ietNumber))
				procs.append(p)
			current_milli_timeLife2 = int(round(time.time() * 1000))
			for i in range(numberOfThreads):
				procs[i].start()
